...
class WalletMigrationTest(BitcoinTestFramework):
    ...
    def test_no_load_after_migration(self):
        ...
        wallet = self.create_legacy_wallet("no_load_after_migration")
        addr = wallet.getnewaddress()
        txid = default.sendtoaddress(addr, 1)
        self.generate(self.master_node, 1)
        bals = wallet.getbalances()
        ...
        self.old_node.unloadwallet("no_load_after_migration")
        shutil.copytree(
            self.old_node.wallets_path / "no_load_after_migration",
            self.master_node.wallets_path / "no_load_after_migration",
            dirs_exist_ok=True,
        )

        migrate_info = self.master_node.migratewallet(wallet_name="no_load_after_migration", load_wallet=False)

        # The wallet should NOT be loaded after migration
        assert "no_load_after_migration" not in self.master_node.listwallets()

        # The returned wallet_name should be populated correctly
        assert_equal(migrate_info["wallet_name"], "no_load_after_migration")

        # The backup path should be reported and exist on disk
        assert os.path.exists(migrate_info["backup_path"])

        # The migrated wallet files should be on disk in SQLite format
        self.assert_is_sqlite("no_load_after_migration", wallet_loaded = False)

        # Load the wallet and verify its state is correct after migration
        self.master_node.loadwallet("no_load_after_migration")
        loaded_wallet = self.master_node.get_wallet_rpc("no_load_after_migration")
