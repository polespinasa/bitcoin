...
def map_data(data):
	...
	for line in data["cmpctblock"]:
		...
		if block in extracted_data.keys():
			extracted_data[block][wtxid] = "unknown-tx"
		else:
			extracted_data[block] = {wtxid: "unknown-tx"}
	txns = {}
...
def main():
	...
	unknown=0
	other=0
	for i in extracted_data.keys():
		for tx in extracted_data[i].keys():
			if "unknown-tx" == extracted_data[i][tx]:
				unknown+=1
			else:
				other+=1
			#print(extracted_data[i][tx])
	#print(extracted_data)
	print("unknown-tx:", unknown)
	print("other:", other)
