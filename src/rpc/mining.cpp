...
static RPCHelpMan generateblock()
{
    return RPCHelpMan{"generateblock",
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    std::map<std::string, UniValue> aod_to_amount;
    UniValue parsed;
    if (!request.params[0].isNull() && parsed.read(request.params[0].get_str())) {
        if (parsed.isArray()) {
            for (std::size_t i = 0; i < parsed.size(); ++i) {
                aod_to_amount[parsed[i].get_str()] = UniValue(0);
            }
        } else if (parsed.isObject()) {
            parsed.getObjMap(aod_to_amount);
        } else {
    ...
    for (const auto& aod : aod_to_amount) {
        ...
        CAmount reward_split = aod.second.getInt<CAmount>();
        if (reward_split < 0) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Error: Parted reward amount cannot be negative");
        }
        if (!getScriptFromDescriptor(script_or_descriptor, coinbase_output_script, error)) {
