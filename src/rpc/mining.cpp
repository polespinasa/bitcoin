...
static RPCMethod generateblock()
{
    return RPCMethod{"generateblock",
        "Mine a block with a set of ordered transactions or mempool transactions to a specified group of addresses or descriptors and optionally the corresponding amount in sats to each one. It return the block hash.\n"
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (size_t i = 0; i < address_or_descriptor.size(); i++) {
        ...
        if (address_or_descriptor_entry.isObject() && !address_or_descriptor_entry.empty()) {
            ...
            custom_reward = AmountFromValue(address_or_descriptor_entry.getValues()[0], 0);
